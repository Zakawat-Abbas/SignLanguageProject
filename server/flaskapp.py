    ...
    with mp_hands.Hands(static_image_mode=True, max_num_hands=2, min_detection_confidence=0.1) as hands:
        while True:
            results = hands.process(
                cv.flip(cv.cvtColor(image, cv.COLOR_BGR2RGB), 1))
...
model = load_model('server\model_SIBI.h5')
...
def predict_image(path_to_image="test_L.jpg"):
    ...
    print(predictions[0])
    predicted_class = list(classes.keys())[list(
        classes.values()).index(predictions[0])]
    # return chr(65 + predictions[0])
